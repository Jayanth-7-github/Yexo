...
class MessageService {
  ...
  static async createMessage(chatId, senderId, messageData) {
    ...
    // Extract replyTo if provided
    const replyToId = meta?.replyToId || meta?.replyTo;
   ...
   * If the same user already reacted with the same emoji, remove it; otherwise add it.
   */
  static async toggleReaction(messageId, userId, emoji) {
    ...
    const idx = reactions.findIndex(
      (r) => r?.emoji === cleanEmoji && r?.user?.equals?.(userId)
    );

    if (idx >= 0) {
      reactions.splice(idx, 1);
    } else {
      reactions.push({ emoji: cleanEmoji, user: userId });
    }
